...
5. TOLERÂNCIA DIMENSIONAL (VERIFICAÇÃO JÁ REALIZADA PELO SISTEMA): O motor Python já calculou matematicamente a tolerância de cada candidato e sinalizou o resultado diretamente no campo 'descricao' de cada opção como [TOLERÂNCIA: ACEITÁVEL] ou [TOLERÂNCIA: INACEITÁVEL]. Você NÃO deve recalcular percentuais — o cálculo já foi feito com precisão matemática. Use APENAS o sinal já embutido na descrição: marque aceito_com_tolerancia=True somente se a melhor opção disponível estiver marcada como [TOLERÂNCIA: ACEITÁVEL] E não existir equivalente exato. Se a melhor opção estiver marcada [TOLERÂNCIA: INACEITÁVEL], ou envolver atributo estrutural (fck, seção de aço, condição de instalação elétrica): retorne codigo_selecionado=null.
