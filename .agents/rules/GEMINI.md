...
| 38 | Bloqueio de API: Fine-Tuning 403 (Tier 1) | A OpenAI bloqueia novas organizações no Tier 1 de usar o Fine-Tuning, retornando erro 403 `training_not_available`. | **Abandono do Fine-Tuning.** Adoção de **Dynamic Few-Shot (RAG)**. A I.A. passa a receber os 3 gabaritos históricos mais similares buscados via `pg_trgm` (Postgres) diretamente no Prompt (`messages`), com custo $0 de setup. |
