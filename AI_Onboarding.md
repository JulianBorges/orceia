...
- **Cuidado Extremo com o Auto-Save (Zustand):** Ao modificar dados na store, lembre-se que funções auxiliares (como `recalculateNumbers`) podem alterar o somatório ou a numeração de *outras* linhas implicitamente (ex: Itens Macro). Sempre faça *Shallow Compare* entre a array antiga e a nova para capturar 100% dos IDs alterados e adicioná-los no `dirtyRowIds`. Caso contrário, a API salvará os dados pela metade.
