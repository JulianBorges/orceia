...
- **Configuração Crítica Cloud Run:** O serviço DEVE obrigatoriamente rodar com a CPU sempre ligada (`--no-cpu-throttling` / CPU Always Allocated). Como a rota despacha chamadas pesadas em `BackgroundTasks`, o estrangulamento padrão congelaria o processamento e mataria a entrega dos dados via SSE.
- **Supabase Connection Pooling (asyncpg):** Para evitar que o auto-scaling do Cloud Run esgote as conexões do Postgres (Erro: *Too Many Clients*), a variável `SUPABASE_DATABASE_URL` deve SEMPRE utilizar o Transaction Pooler (porta `6543`), nunca a conexão direta (5432).
...
- **Pragmatismo no TypeScript (Vercel):** Para viabilizar a transição da V2 (cherry-picking), o `next.config.mjs` e as compilações intencionalmente ignoram erros de Lint e tipagem (`any`). Não perca tokens/tempo tentando refatorar tipos do React a menos que estritamente necessário para uma feature nova. O foco é MVP.
