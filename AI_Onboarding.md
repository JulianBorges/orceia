...
### Atualiza��es Arquiteturais Recentes (Auditoria Motor IA)
1. **Pinecone**: Utiliza Lazy Initialization para n�o derrubar inst�ncias serverless no boot.
2. **Concorr�ncia**: Sem�foro ass�ncrono limitado a 3 por worker para evitar Rate Limit 429 da OpenAI durante auto-scaling.
3. **Curva ABC**: Totalmente desconectada do LLM. O c�lculo da Curva ABC deve ser feito matematicamente no backend ou frontend no momento da exporta��o do relat�rio.
4. **UX do SSE**: O Redis entrega pacotes de 2 em 2 (n�o mais 10 em 10) para uma UI fluida.
