...
3.  **Auto-Save Inteligente O(1):** Crie um *Dirty Checking*. Ao alterar uma linha, adicione o ID dela a um Array `dirtyRowIds`. Um hook *debounced* de 3 segundos monitora a flag genérica `isDirty`. Não faça parse e não compare JSON gigantes na memória do usuário.
